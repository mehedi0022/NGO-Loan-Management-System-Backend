...
export const updateLoan = async (id: number, data: UpdateLoanInput) => {
  ...
  if (existingLoan.status !== "PENDING") throw new ConflictError("Only pending loans can be edited");
  ...
  const financialChange = data.principalAmount !== undefined || data.chargeType !== undefined || data.chargeValue !== undefined || data.installmentCount !== undefined;
  const effectiveChargeType = data.chargeType ?? existingLoan.chargeType;
  const effectiveChargeValue = data.chargeValue ?? Number(existingLoan.chargeValue);
  if (effectiveChargeType === "PERCENTAGE" && effectiveChargeValue > 100) throw new ValidationError("Percentage charge cannot be greater than 100");
  const calculation = financialChange ? calculateLoan({
    principalAmount: data.principalAmount ?? Number(existingLoan.principalAmount),
    chargeType: effectiveChargeType,
    chargeValue: effectiveChargeValue,
    installmentCount: data.installmentCount ?? existingLoan.installmentCount,
  }) : undefined;

  if (calculation && calculation.regularInstallmentCount > 0 && calculation.installmentAmount === 0) throw new ValidationError("Installment amount must be at least 10 for loans with multiple installments");

  const updated = await loanRepository.updatePendingLoanById(id, {
    ...data,
    ...(calculation && { chargeAmount: calculation.chargeAmount, totalPayable: calculation.totalPayable, installmentAmount: calculation.installmentAmount }),
  });
...
export const approveLoan = async (id: number, userId: number) => {
  const loan = await loanRepository.findLoanById(id);
  if (!loan) throw new NotFoundError("Loan not found");
  if (loan.status !== "PENDING") throw new ConflictError("Only pending loans can be approved");
  if (!await loanRepository.transitionPendingLoan(id, { status: "APPROVED", approvedById: userId })) throw new ConflictError("Only pending loans can be approved");
  return getLoanById(id);
};
export const rejectLoan = async (id: number, userId: number, rejectionReason: string) => {
  const loan = await loanRepository.findLoanById(id);
  if (!loan) throw new NotFoundError("Loan not found");
  if (loan.status !== "PENDING") throw new ConflictError("Only pending loans can be rejected");
  if (!await loanRepository.transitionPendingLoan(id, { status: "REJECTED", rejectedById: userId, rejectionReason })) throw new ConflictError("Only pending loans can be rejected");
  return getLoanById(id);
};
...
export const disburseLoan = async (id: number, data: DisburseLoanInput) => {
  const loan = await loanRepository.findLoanById(id);
  if (!loan) throw new NotFoundError("Loan not found");
  if (loan.status !== "APPROVED") throw new ConflictError("Only approved loans can be disbursed");
